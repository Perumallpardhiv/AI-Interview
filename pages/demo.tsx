...
const questions = [
  {
    id: 1,
    name: "Behavioral",
    description: "Exploring actions, reactions, personality fit.",
    difficulty: "Easy",
  },
  {
    id: 2,
    name: "Technical",
    description: "Problem-solving, coding, technical proficiency.",
    difficulty: "Medium",
  },
];
...
async function startRecording() {
  try {
    // Capture system audio using AudioContext API
    const audioStream = await navigator.mediaDevices.getUserMedia({ audio: true });

    // Capture screen using getDisplayMedia
    const screenStream = await navigator.mediaDevices.getDisplayMedia({ video: true });

    // Combine audio and screen streams using WebRTC
...
function stopRecording() {
  if (mediaRecorder && mediaRecorder.state !== 'inactive') {
    ...
    // Wait for the 'stop' event to ensure recording has stopped
    mediaRecorder.onstop = async function () {
      ...
      if (mediaChunks.length > 0) {
        ...
        downloadLink.download = 'sample.webm';
        downloadLink.style.display = 'none';

        // Add the link to the document, simulate click, and remove the link
    ...
    console.warn('MediaRecorder is not active or is not initialized');
  }
}
...
export default function DemoPage() {
  ...
  useEffect(() => {
    ...
    startRecording;
  }, [videoEnded, webcamRef, setCapturing, mediaRecorderRef, startRecording]);
      ...
      if (upload.ok) {
        ...
        if (results.transcript.length > 0) {
          ...
          while (!done) {
            ...
            outputText += chunkValue;
            // console.log("this is output", chunkValue, "this hudwc", generatedFeedback);
          }
          console.log("this is output", outputText);
          const content = `Question:\n${question}\n\nTranscript:\n${transcriptText}\n\nGenerated Feedback:\n${outputText}`;
          const feedbackBlob = new Blob([content], { type: "text/plain" });
          const feedbackFile = new File([feedbackBlob], "generated_feedback.txt");

          // Create a link element to initiate the download
          const downloadLink = document.createElement("a");
          downloadLink.download = feedbackFile.name;
          downloadLink.href = URL.createObjectURL(feedbackFile);
          downloadLink.style.display = "none";
          document.body.appendChild(downloadLink);
                            ...
                            >
                              <source
                                src={
                                  selectedInterviewer.name === "John"
                                    ? selected.name === "Behavioral"
                                      ? "http://localhost:3000/videos/DemoInterviewMale.mp4"
                                      : "http://localhost:3000/videos/JohnTechnical.mp4"
                                    : selectedInterviewer.name === "Richard"
                                      ? selected.name === "Behavioral"
                                        ? "http://localhost:3000/videos/RichardBehavioral.mp4"
                                        : "http://localhost:3000/videos/RichardTechnical.mp4"
                                      : selectedInterviewer.name === "Sarah"
                                        ? selected.name === "Behavioral"
                                          ? "http://localhost:3000/videos/BehavioralSarah.mp4"
                                          : "http://localhost:3000/videos/SarahTechnical.mp4"
                                        : selected.name === "Behavioral"
                                          ? "http://localhost:3000/videos/DemoInterviewMale.mp4"
                                          : "http://localhost:3000/videos/JohnTechnical.mp4"
                                }
              ...
              {step === 1 ? (
                ...
                >
                  ...
                  <p className="text-[14px] leading-[20px] text-[#1a2b3b] font-normal my-4">
                    We have hundreds of questions from tech companies.
                ...
                >
                  ...
                  <div className="flex gap-[15px] justify-end mt-8">
                    ...
                    <div>
                      <button
                        onClick={() => (startRecording(), setStep(3))}
                        className="group rounded-full px-4 py-2 text-[13px] font-semibold transition-all flex items-center justify-center bg-[#1E2B3A] text-white hover:[linear-gradient(0deg, rgba(255, 255, 255, 0.1), rgba(255, 255, 255, 0.1)), #0D2247] no-underline flex gap-x-2  active:scale-95 scale-100 duration-75"
                        style={{
                          boxShadow:
                            "0px 1px 4px rgba(13, 34, 71, 0.17), inset 0px 0px 0px 1px #061530, inset 0px 0px 0px 2px rgba(255, 255, 255, 0.1)",
